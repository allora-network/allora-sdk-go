...
{{range .Methods}}
func (c *{{ $.ModuleName | title }}GRPCClient) {{ .Name }}(ctx context.Context, req *{{ $.PackageName }}.{{ .RequestType }}) (*{{ $.PackageName }}.{{ .ResponseType }}, error) {
    resp, err := c.client.{{ .Name }}(ctx, req)
    return resp, errors.Wrap(err, "while calling AuthGRPCClient.{{ .Name }}")
}
{{end}}