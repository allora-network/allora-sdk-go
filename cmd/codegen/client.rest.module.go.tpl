...
func (c *{{ $.ModuleName | title }}RESTClient) {{ .Name }}(ctx context.Context, req *{{ $.PackageName }}.{{ .RequestType }}, opts ...config.CallOpt) (*{{ $.PackageName }}.{{ .ResponseType }}, error) {
    ...
    if err != nil {
        return nil, errors.Wrap(err, "while calling {{ $.ModuleName | title }}RESTClient.{{ .Name }}")
    }
    return resp, nil
}
